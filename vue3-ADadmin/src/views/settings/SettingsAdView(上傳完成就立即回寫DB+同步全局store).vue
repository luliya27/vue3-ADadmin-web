...
const onUpload = async (field: 'syslogo' | 'sysbackgroundimg') => {
    ...
    input.onchange = async () => {
        ...
        try {
            ...
            // ✅ 上傳後立即寫回 DB（不需要再點儲存）
            // adsettings 預期只有一筆，若 id 尚未載入則使用 1
            await updateAdSettings({ ...form, id: (form as any).id || 1 })

            // ✅ 同步到全局設定（Topbar / Login 會立即套用）
            adSettingsStore.setSettings({ ...(form as any) })

            message.value = '上傳成功，已立即套用'
            messageType.value = 'success'
        } catch (err) {
            console.error(err)
            message.value = '上傳或套用失敗'

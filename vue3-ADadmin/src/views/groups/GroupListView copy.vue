<template>
    <div class="group-list-page">
        ...
        <section class="table-wrapper">
            <table class="group-table">
                ...
                <tbody>
                    ...
                    <tr v-for="group in groups" :key="group.groupname" @dblclick="openEdit(group)">
                        <td class="mono link">{{ group.groupname }}</td>
                        <td>{{ group.description || '-' }}</td>
                        <td>
                            <span class="type-pill" :class="group.grouptype">
                                {{ groupTypeLabel(group.grouptype) }}
                            </span>
                        </td>
                        <td>
                            <button class="link-btn" @click="openMembersDialog(group)">
                                查看
                            </button>
                        </td>
                    </tr>
        ...
        <!-- 建立 -->
        <CreateGroupModal v-if="showCreate" @close="showCreate = false" @submit="submitCreate" />

        <!-- 修改 / 刪除 -->
        <EditGroupModal v-if="showEdit && editing" :group="editing" @close="closeEdit" @submit="submitEdit"
            @delete="openDeleteConfirm" />

        <!-- 刪除防呆 -->
        <DeleteConfirmModal v-if="showDeleteConfirm" :groupname="editing?.groupname || ''"
            @cancel="showDeleteConfirm = false" @confirm="confirmDelete" />
    </div>
    <!-- 群組成員彈窗 -->

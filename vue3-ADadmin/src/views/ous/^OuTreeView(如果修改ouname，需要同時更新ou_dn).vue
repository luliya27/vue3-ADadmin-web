<template>
    <div class="ou-page">
        ...
        <CreateOuModal v-if="showCreate" :dc-dn="dcDn" :ou-options="ous" @close="showCreate = false"
            @submit="submitCreate" />
        <!-- 修改 / 刪除 OU -->
        <EditOuModal v-if="showEdit && editingOu" :ou="editingOu" :ou-options="ous" @close="closeEdit" @submit="submitEdit"
...
const submitEdit = async (data: { ouname: string; description: string; childOuName?: string }) => {
    ...
    // 更新父 OU 的 ouname、description 和 ou_dn
    const updatePayload: any = {}
    
    // 一定要传 description
    updatePayload.description = data.description
    
    // 如果修改了 ouname，需要同时更新 ou_dn
    if (data.ouname !== editingOu.value.ouname) {
        updatePayload.ouname = data.ouname
        // ou_dn 基于 parent_dn 构建
        if (!editingOu.value.parent_dn) {
            // 根 OU
            updatePayload.ou_dn = `OU=${data.ouname},DC=corp,DC=example,DC=com`
        } else {
            // 子 OU
            updatePayload.ou_dn = `OU=${data.ouname},${editingOu.value.parent_dn}`
        }
    }

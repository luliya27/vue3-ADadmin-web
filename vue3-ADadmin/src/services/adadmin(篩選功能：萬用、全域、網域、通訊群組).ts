...
// 前端用的篩選類別
// ✅ type=security 代表三種 security-* 都要
...
export interface GroupQuery {
  q?: string
  // type?: 'all' | 'security-global' | 'security-domainlocal' | 'security-universal' | 'distribution'
...
export async function fetchGroups(params?: GroupQuery): Promise<Group[]> {
  ...
  // 後端支援兩種：type=security 或 types=...
  // 這裡用 types 來做「安全性群組 = 三種類型」聚合
  if (type && type !== 'all') {
    if (type === 'security') {
      query.types = ['security-global', 'security-domainlocal', 'security-universal'].join(',')
    } else {
      query.type = type
    }
  }

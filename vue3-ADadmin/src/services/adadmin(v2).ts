...
export type GroupItem = {
  ...
  grouptype: 'security-global' | 'security-domainlocal' | 'security-universal' | 'distribution'
}

export async function fetchGroups(params?: { q?: string; type?: string }): Promise<GroupItem[]> {
  const res = await http.get('/api/groups', { params })
  return res.data.data
}
...
export async function createGroup(payload: {
  ...
  const res = await http.post('/api/groups', payload)
  return res.data
}

export async function updateGroup(id: number, payload: {
  groupname: string
  description?: string
  grouptype: GroupItem['grouptype']
}) {
  const res = await http.put(`/api/groups/${id}`, payload)
  return res.data
}
...
export interface GroupQuery {
  q?: string
  type?: 'all' | 'security' | 'distribution'
}

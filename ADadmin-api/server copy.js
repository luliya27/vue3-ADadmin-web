  ...
  db.get(`SELECT dc_dn FROM adsettings LIMIT 1`, [], (err, s) => {
    ...
    db.get(findParentSql, [parentName], (pErr, parent) => {
      ...
      const insertChildSql = `
        ...
        VALUES (?, ?, ?, ?, 0)
      `
      db.run(insertChildSql, [ou_dn, childName, desc, parent_dn], function (insErr) {
        if (insErr) {
        ...
        db.run(`UPDATE ous SET parentou = 1 WHERE id = ?`, [parent.id], (uErr) => {
          ...
          return res.json({
            success: true,
            data: { id: this.lastID, ou_dn, ouname: childName, description: desc, parent_dn, parentou: 0 },
          })
        })
      })
    })
...
app.patch('/api/ous/:id', (req, res) => {
  ...
  db.get(`SELECT dc_dn FROM adsettings LIMIT 1`, [], (err, setting) => {
    ...
    db.get(`SELECT * FROM ous WHERE id = ?`, [id], (err2, ou) => {
      ...
      let newParentDn = parent_dn ?? ou.parent_dn
      let newParentou = (typeof parentou === 'number') ? parentou : ou.parentou

      // 如果前端傳來 ouname 或 ou_dn，使用前端的值
      if (ou_dn) {
        newOuDn = ou_dn
      } else {
        // 否則根據 newOuname 和 parent_dn 計算 ou_dn
      ...
      db.run(sql, [
        ...
        res.json({ success: true, data: { id, ouname: newOuname, ou_dn: newOuDn, parent_dn: newParentDn, parentou: newParentou } })
      })
    })
  })
})

...
CREATE TABLE IF NOT EXISTS ous (
  id           INTEGER PRIMARY KEY,                    -- 你假資料用 1,2,3... 這裡用 INTEGER 最合理
  ou_dn        TEXT NOT NULL UNIQUE,                   -- 完整 DN
  ouname       TEXT NOT NULL,                          -- OU 名稱
  description  TEXT,                                   -- OU 別名(部門)

  parentou     INTEGER NOT NULL DEFAULT 0 CHECK(parentou IN (0, 1)),  -- 0/1：是否啟用子層(有上層 OU)
  parent_dn    TEXT NULL,                              -- 父 OU DN（你 INSERT 有用到）

      ...
      db.get(findChildSql, [childName], (cErr, existingChild) => {
        ...
        if (existingChild) {
          ...
          const updateChildSql = `
            UPDATE ous
            SET ou_dn = ?, parent_dn = ?, parent_id = ?, parentou = 0, updated_at = datetime('now')
            WHERE id = ?
          `
          db.run(updateChildSql, [newOuDn, parent_dn, parent.id, existingChild.id], function (uErr) {
            if (uErr) {
            ...
            db.run(`UPDATE ous SET parentou = 1, updated_at = datetime('now') WHERE id = ?`, [parent.id], (pErr) => {
              ...
              return res.json({
                success: true,
                data: { id: existingChild.id, ou_dn: newOuDn, ouname: childName, description: existingChild.description || '', parent_dn, parent_id: parent.id, parentou: 0 },
              })
            })
          })
        } else {
...
//   })
// })
// 也能改 parent_dn / parentou
app.patch('/api/ous/:id', (req, res) => {
  const { id } = req.params
  const { description, parent_dn, parentou } = req.body
